...
└── sandbox/                  # Sandbox implementation
    ...
    ├── loopback-shim.ts      # Linux in-sandbox loopback-first listener (with vendor/loopback-shim/)
...
- `network.allowLocalBinding` - Allow binding to local ports (boolean, default: false). **macOS only** — see "Loopback on Linux" below.

**Loopback on Linux.** Under `bwrap --unshare-net` the sandboxed process gets its own network namespace, so its `127.0.0.1` is *not* the host's loopback: a server the sandbox itself binds there is reachable only from inside the sandbox (issue #165), and a direct connect from inside can never reach a service running on the host. Both directions therefore run through the proxy, and the in-sandbox listener decides which loopback a request means.

- **A loopback destination on the allowlist** routes loopback through the proxy: `localhost` / `127.0.0.1` / `::1` are dropped from the child's `NO_PROXY`, so an env-honoring client (`curl`, `wget`, `fetch`, …) hands those requests to the proxy instead of dialing its own empty loopback namespace. The proxy connects from the host and is not namespaced.
- **The listener then tries the sandbox's own loopback first.** A dev server the *sandboxed* process started listens on the sandbox's `127.0.0.1`, which the host proxy cannot reach, while a server started *outside* the sandbox listens on the host's, which a direct connect inside cannot reach. Since a client cannot tell which side its server is on, the in-sandbox HTTP listener (`vendor/loopback-shim/shim.sh`, run per connection by socat) probes the sandbox's own `127.0.0.1:<port>` and relays there if something answers, otherwise bridges to the host proxy as before. Both workflows work under one allowlist.
  - `CONNECT` (HTTPS, and `ssh` via `GIT_SSH_COMMAND`) is completed by the listener itself when a sandbox-local server answers, so the client's TLS runs end-to-end against that server; otherwise the host proxy owns the tunnel and its allow/deny lists apply.
  - Local-first is used **only** for a loopback destination named by a `CONNECT` authority or an absolute-form request-URI — never for a hostname the allowlist has to vet, so policy enforcement for every other destination is unchanged. The local hop is a connect to the sandbox's own loopback, i.e. the reach a direct dial always had.
  - Traffic that is not HTTP (a SOCKS greeting, TLS, SSH) is forwarded straight to the host proxy without being parsed. The SOCKS listener stays a plain bridge, so a client that speaks SOCKS to it is byte-transparent as before.
  - Only the loopback *probe* is added to a request, and only for a loopback destination (refused in microseconds); every other request takes the same path as before.
- **Spelling is not significant.** An entry of `localhost`, `*.localhost`, `127.0.0.1`, `127.0.0.2`, or `[::1]` admits any of the others (`127.0.0.1` is the same server as `localhost`), and a `:port` suffix still restricts to that port. So a dev server or local API on the host needs only `"allowedDomains": ["localhost"]` (or `["127.0.0.1:11434"]` for one port); an empty allowlist leaves loopback blocked like any other destination, and a `deniedDomains` loopback entry still wins.
- **Clients that ignore the proxy environment** (a raw socket, or anything hardcoding a direct connect) can only reach the sandbox's own loopback — that is inherent to the namespace split, not a policy choice. Chromium is a special case worth knowing: it ignores `HTTP_PROXY`, so a browser reaching a host-side server needs the browser's own proxy configuration (`PUPPETEER_PROXY` for the bundled one); reaching a sandbox-side server works without it.
- **Host reaching a server inside the sandbox** is not bridged on Linux (`allowLocalBinding` is macOS-only, since Seatbelt can allow `bind()` while sharing the host network stack). Use `excludedCommands`, or run the listener outside the sandbox.

macOS is unaffected by all of this: Seatbelt shares the host network stack, so the child's loopback *is* the host's loopback, `allowLocalBinding` governs binding, and the `NO_PROXY` loopback entries stay in place.
...
`srt-proxy-relay` is installed next to `srt` (npm's `bin`: `vendor/srt-proxy-relay/srt-proxy-relay.mjs`, node builtins only — run it with `bun` if the sandbox has no `node`). Reports go to stderr whenever it supervises a client, keeping that client's stdout — an MCP server's JSON-RPC stream — clean.
