...
**Loopback on Linux.** Under `bwrap --unshare-net` the sandboxed process gets its own network namespace, so its `127.0.0.1` is *not* the host's loopback: a server the sandbox itself binds there is reachable only from inside the sandbox (issue #165), and a direct connect from inside can never reach a service running on the host. There is accordingly no direct loopback path in or out; both directions go through the proxy.

- **Host services from inside the sandbox** work when the destination is on the allowlist. Loopback destinations are matched as one destination regardless of spelling — an entry of `localhost`, `*.localhost`, `127.0.0.1`, `127.0.0.2`, or `[::1]` admits any of the others (`127.0.0.1` is the same server as `localhost`), and a `:port` suffix still restricts to that port. When the allowlist names any loopback destination, `localhost` / `127.0.0.1` / `::1` are dropped from the child's `NO_PROXY` so clients hand those requests to the proxy instead of dialing their own empty loopback namespace; the proxy connects from the host and is not namespaced. Example: a dev server or local API on the host plus `"allowedDomains": ["localhost"]` (or `["127.0.0.1:11434"]` for one port). An empty allowlist leaves loopback blocked like any other destination.
